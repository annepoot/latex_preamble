...
\ifx\XeTeXrevision\undefined
  % Compilation not done with xelatex
  \usepackage{amsfonts}
\else
  % Compilation done with xelatex
  \@ifpackageloaded{unicode-math}
    {
      % unicode-math already loaded
      \setmathfont{Latin Modern Math}[range=\mathcal]
    }
    {
      % unicode-math not yet loaded
...
\@ifpackageloaded{unicode-math}
  ...
  {
    \newcommand*{\fmtvector}[1]{{\mathbf{#1}}}
  }
\makeatother
...
\providecommand{\vlambda}{\fmtvector{\lambda}}
...
\@ifpackageloaded{unicode-math}
  {
    \newcommand*{\fmtmatrix}[1]{{\symbfit{#1}}}
  }
  {
    \newcommand*{\fmtmatrix}[1]{{\mathbf{#1}}}
  }
\makeatother
...
\newcommand*{\banachadjoint}[1]{#1'}
\newcommand*{\hilbertadjoint}[1]{\super{#1}{*}}
\newcommand*{\adjoint}[1]{\banachadjoint{#1}}
\newcommand*{\inverse}[1]{\super{#1}{-1}}
\newcommand*{\pseudoinverse}[1]{\super{#1}{+}}
\newcommand*{\squaredinverse}[1]{\super{#1}{-2}}

% Abbreviations
\providecommand{\T}[1]{\transpose{#1}}
\providecommand{\H}[1]{\hermitian{#1}}
\providecommand{\badj}[1]{\banachadjoint{#1}}
\providecommand{\hadj}[1]{\hilbertadjoint{#1}}
...
% Combined operators
\providecommand{\Tinv}[1]{\super{#1}{-\transposesymbol}}
\providecommand{\Tpinv}[1]{\super{#1}{+\transposesymbol}}
...
% typo is on purpose! \angles gets overwritten by unicode math (I think)
\DeclarePairedDelimiter{\angs}{\langle}{\rangle}
\DeclarePairedDelimiter{\singlepipes}{\lvert}{\rvert}
\DeclarePairedDelimiter{\doublepipes}{\lVert}{\rVert}
...
\NewDocumentCommand{\seminorm}{s o m}{
  \IfBooleanTF{#1}
    {\singlepipes*{#3}\IfValueT{#2}{_{#2}}}
    {\singlepipes{#3}\IfValueT{#2}{_{#2}}}
}

\NewDocumentCommand{\absolute}{s m}{
  \IfBooleanTF{#1}
    {\singlepipes*{#2}}
    {\singlepipes{#2}}
}
